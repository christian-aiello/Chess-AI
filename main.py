...
class Pawn(Piece):
    ...
    def get_moves(self, board):
        ...
        if self.color: captures, two_squares, one_square, starting, end = [(-1, 1), (-1, -1)], -2, -1, 6, 0
        else: captures, two_squares, one_square, starting, end = [(1, 1), (1, -1)], 2, 1, 1, 7
        ...
        for (r, c) in captures:
            ...
            ec = self.col + c
            if in_board(er, ec):
                if board.board[er][ec] != 0 and (board.board[er][ec]).color != self.color:
                    if er == end:
                        moves.append(((self.row, self.col), (er, ec), 'q'))
                        moves.append(((self.row, self.col), (er, ec), 'r'))
                        moves.append(((self.row, self.col), (er, ec), 'b'))
        ...
        if self.color and board.turn:
            if wk:
                if (board.board[7][5] == 0) and (board.board[7][6] == 0):
                    if (7, 5) not in board.get_captured(opponent=True) and (7, 6) not in board.get_captured(opponent=True):
                        moves.append(((self.row, self.col), (7, 6)))
            if wq:
                if (board.board[7][3] == 0) and (board.board[7][2] == 0) and (board.board[7][1] == 0):
                    if (7, 3) not in board.get_captured(opponent=True) and (7, 2) not in board.get_captured(opponent=True):
                        moves.append(((self.row, self.col), (7, 2)))
        elif not self.color and not board.turn:
            if bk:
                if (board.board[0][5] == 0) and (board.board[0][6] == 0):
                    if (0, 5) not in board.get_captured(opponent=True) and (0, 6) not in board.get_captured(opponent=True):
                        moves.append(((self.row, self.col), (0, 6)))
            if bq:
                if (board.board[0][3] == 0) and (board.board[0][2] == 0) and (board.board[0][1] == 0):
                    if (0, 3) not in board.get_captured(opponent=True) and (0, 2) not in board.get_captured(opponent=True):
...
class Board:
    ...
    def is_checkmate(self):
        ...
        for row in range(8):
            for col in range(8):
                if type(self.board[row][col]).__name__ == 'King' and (self.board[row][col]).color == self.turn:
    ...
    def generate_board(self, fen='rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1'):
        fen = fen.split(' ')
    ...
    def make_move(self, start, end, promotion=None):
        ...
        if type(self.board[sr][sc]).__name__ == 'King':
            if (self.board[sr][sc]).color:
                if abs(sc-ec) == 2:
                    if ec == 6 and type(self.board[7][7]).__name__ == 'Rook' and (self.board[7][7]).color:
                        self.board[7][5] = self.board[7][7]
                        self.board[7][7] = 0
                        (self.board[7][5]).row = 7
                        (self.board[7][5]).col = 5
                    if ec == 2 and type(self.board[7][0]).__name__ == 'Rook' and (self.board[7][0]).color:
                ...
                if abs(sc-ec) == 2 and (sr, sc):
                    if ec == 6 and type(self.board[0][7]).__name__ == 'Rook' and not (self.board[0][7]).color:
                        self.board[0][5] = self.board[0][7]
                        self.board[0][7] = 0
                        (self.board[0][5]).row = 0
                        (self.board[0][5]).col = 5
                    if ec == 2 and type(self.board[0][0]).__name__ == 'Rook' and not (self.board[0][0]).color:
    ...
    def validate_move(self, start, end, promotion=None):
        ...
        for row in board_copy.board:
            for col in row:
                if col != 0:
                    if type(col).__name__ == 'King' and (col.color == (not board_copy.turn)):
...
def main():
    ...
    while True:
        ...
        move = input()
        if len(move) == 5 and move[2] == ' ' and move[1] in string.digits and move[4] in string.digits and move[0] not in string.digits and move[3] not in string.digits:
            if (algebraic(move)[0], algebraic(move)[1]) in game.get_all_moves():
                game.make_move(*algebraic(move))
            elif (algebraic(move)[0], algebraic(move)[1]) in [i[0:2] for i in game.get_all_moves()]:
                move = algebraic(move)
